...
def get_severity(percent, mode="lung"):
    if mode == "lung":
        ...
        if percent < 0.5: return "Negative", "severity-mild"
        if percent < 2: return "Small Tumor", "severity-moderate"
...
def process_image(uploaded_file, mode_id="lung", threshold=0.3, from_bytes=None):
    ...
    if mode_id == "lung":
        ...
        # Post-processing
        pred_mask = (torch.sigmoid(pred[0]).cpu().numpy() > threshold).astype(np.uint8).squeeze()
        # For visualization, we use the original BGR image converted to RGB
        image = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY) # Still keep a gray version for verification functions
